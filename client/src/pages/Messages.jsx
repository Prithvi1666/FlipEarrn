...
const Messages = () => {
  ...
  const filteredChats = useMemo(()=>{
    const quwey =searchQuery.toLowerCase();
    return chats.filter((chat)=>{
      const chatUser = chat.chatUserId === user?.id ? chat?.ownerUser : chat?.chatUser;

      return chat.listing?.title?.toLowerCase().includes(quwey) || chatUser?.name?.toLowerCase().includes(quwey);
    })
  },[chats, searchQuery])
    ...
    <div className='mx-auto min-h-screen px-6 md:px-16 lg:px-24 xl:px-32' >
      <div className='py-10'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-gray-600'>Chat whith buyers and sellers</p>
        </div>
        ...
        <div className='relative max-w-xl mb-8'>
          ...
          className='w-full pl-10 pr-4 py-2 borrder border-gray-300 rounded-lg focus:outline-indigo-500'/>
        </div>
        {/* chat List */}
        {loading ? (
          ...
          <div className='bg-white rounded-lg shadow-xs border border-gray-200 p-16 text-center '>
            ...
            <p className='text-gray-600'>
              {searchQuery ? "try a differnt search term" : 'Satrt a conversation by viewing a listing and clicking "Chat with Seller"'}
            </p>
          </div>
        ): (
                ...
                key={chat.id} className='w-full p-4 hover:bg-gray-50 transition-colors text-left'>
                  <div className='flex items-start space-x-4'>
                    ...
                    <div className='flex-1 min-w-0 '>
                      ...
                      <p className={`text-sm truncate ${!chat.isLastMessageRead && chat.lastMessageSenderId !== user?.id ? 'text-indigo-600 font-medium'
                        : "text-gray-500 "}`}>{chat.lastMessage || 'No messages yet'}</p>
                    </div>
                  </div>
                </button>
              )
