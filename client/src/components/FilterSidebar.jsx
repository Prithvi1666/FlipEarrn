...
const FilterSidebar = ({showFilterPhone, setShowFilterPhone, filters, setFilters}) => {
  ...
  return (
    <div className={`${showFilterPhone ? "max-sm:fixed" : "max-sm:hidden"} z-100 max-sm:h-screen max-sm:overflow-scroll bg-white rounded-lg shadow-sm border
    border-gray-200 h-fit sticky top-24 md:min-w-[300px]`}>
            ...
            {expandedSections.status && (
               <div className='space-y-3'>
                 <label className='flex items-center space-x-2 cursor-pointer'>
                    <input type="checkbox" checked={filters.verified} onChange={(e) => onFiltersChange({...filters, verified: e.target.checked})} />
                    <span className='text-sm text-gray-700'>Verified accounts only</span>
                 </label>
                 <label className='flex items-center space-x-2 cursor-pointer'>
                    <input type="checkbox" checked={filters.monetized} onChange={(e) => onFiltersChange({...filters, monetized: e.target.checked})} />
