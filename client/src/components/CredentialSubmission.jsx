    ...
    <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-[320px] flex flex-col'>
        ...
        <form onSubmit={handleSubmission} className='flex flex-col items-start gap-4 p-4 overflow-y-scroll'>
            {credential.map((cred, index)=>(
                <div key={cred.name} className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2'>
                    <label className='text-sm font-medium text-gray-800 '>{cred.name}</label>
                    <input type="text" value={cred.value} onChange={(e)=>setCredential((prev)=>prev.map((c, i)=>(i=== index ? {...c,
                        value: e.target.value} : c)))} className='w-full px-2 py-1.5 text-sm border border-gray-300 rounded outline-indigo-400'/>
                        <X className='w-5 h-5 text-gray-500 hover:text-gray-700 cursorr-pointer' onClick={()=> setCredential((prev)=>prev.filter
                        ((_, i)=> i !== index))}/>
                </div>
            ))}
            {/* Add More fields */}
            <div className='flex items-center gap-2'>
                <input type="text" value={newField} onChange={(e)=>setNewField(e.target.value)} placeholder='field name ... 'className="outline-none border border-gray-200 "/>
